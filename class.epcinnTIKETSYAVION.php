...
$NOTA_DE_CREDITO_COMPRA,
 $ADJUNTAR_FACTURA_PDF1){
	$MONTO_TOTAL_COTIZACION_ADEUDO  = trim(str_replace(array(',', '$'), '', $MONTO_TOTAL_COTIZACION_ADEUDO));

		$MONTO_FACTURA  = trim(str_replace(array(',', '$'), '', $MONTO_FACTURA));

		$MONTO_PROPINA  = trim(str_replace(array(',', '$'), '', $MONTO_PROPINA));

		$MONTO_DEPOSITAR  = trim(str_replace(array(',', '$'), '', $MONTO_DEPOSITAR));

		$MONTO_DE_COMISION  = trim(str_replace(array(',', '$'), '', $MONTO_DE_COMISION));		
		$conn = $this->db();
		//$existe = $this->revisar_altaeventos();
		$session = isset($_SESSION['idem'])?$_SESSION['idem']:'';  
		if($session != ''){
				$adjuntarFacturaPdfUpdate = '';

		if($ADJUNTAR_FACTURA_PDF1 != ''){

			$adjuntarFacturaPdfUpdate = ", ADJUNTAR_FACTURA_PDF1 = '".$ADJUNTAR_FACTURA_PDF1."'";
		...
		tipo_documento = '".$tipo_documento."'".$adjuntarFacturaPdfUpdate."
