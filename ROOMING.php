...
<div class="progress" style="width: 25%;">
        ...
        <div class="card">
          <div class="card-body">
          <?php if($conexion->variablespermisos('','ROOMING','guardar')=='si' and $var_bloquea_fecha=='no'){ ?>
 ...
 <button  style="float:right"  class="btn btn-sm btn-outline-success px-5"   type="button" id="GUARDAR_ROOMING">GUARDAR</button><div style="
 position: absolute;
    top: 50%; 
    right: 50%;
...
id="mensajeROOMING"/></th></tr><?php } ?>
                  ...
                  <?php if($conexion->variablespermisos('','ROOMING','email')=='si' and $var_bloquea_fecha=='no'){ ?>
                  <form name="form_emai_rooming" id="form_emai_rooming">
				  <tr>
             <td ><textarea  placeholder="ESCRIBE AQUÍ TUS CORREOS SEPARADOS POR PUNTO Y COMA EJEMPLO: NOMBRE@CORREO.ES;NOMBRE@CORREO.ES" style="width: 500px;" name="EMAIL_rooming" id="EMAIL_rooming" class="form-control" aria-label="With textarea"><?php echo $EMAIL_rooming; ?></textarea>
            <button class="btn btn-sm btn-outline-success px-5"  type="button" id="BUTTON_email_rooming">ENVIAR POR EMAIL</button></td>   
...
{	
	$urlADJUNTO_ROOMING = $conexion->descargararchivo($row["ADJUNTO_ROOMING"]);
?>

<tr style='background:#f5f9fc;text-align:center'>
<td style="text-align:center" >
<input type="checkbox" style="width:15%" class="form-check-input" name="rooming[]" id="rooming" value="<?php echo $row["id"]; ?>"/> </td>
<td ><?php echo $row["DOCUMENTO_ROOMING"]; ?></td>
<td ><?php echo $urlADJUNTO_ROOMING; ?></td>
<td ><?php echo $row["OBSERVACIONES_ROOMING"]; ?></td>
<td ><?php echo $row["FECHA_ROOMING"]; ?></td>


<td>
<?php if($conexion->variablespermisos('','ROOMING','modificar')=='si' and $var_bloquea_fecha=='no'){ ?><input type="button" name="view" value="MODIFICAR" id="<?php echo $row["id"]; ?>" class="btn btn-info btn-xs view_dataROOMINGmodifica" /><?php } ?></td>
<td><?php if($conexion->variablespermisos('','ROOMING','borrar')=='si' and $var_bloquea_fecha=='no'){ ?><input type="button" name="view2" value="BORRAR" id="<?php echo $row["id"]; ?>" class="btn btn-info btn-xs view_dataROMMINGborrar" /><?php } ?></td>
</tr>
<?php
}
?>
