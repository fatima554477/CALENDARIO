...
while($ROWap=mysqli_fetch_array($QUERYAvionp)){
	...
	$VarCOMPROBACION = 'SELECT  subTotal , UUID, MONTO_DEPOSITAR FROM 07COMPROBACION LEFT JOIN 07XML ON 07COMPROBACION.id = 07XML.`ultimo_id` where ( 07COMPROBACION.NUMERO_EVENTO = "'.$NUMERO_EVENTO.'" ) ';
...
while($ROWd=mysqli_fetch_array($QUERYCOMPROBACION)){
	...
	$VarCOMPROBACIONpropina = 'SELECT  MONTO_PROPINA ,IMPUESTO_HOSPEDAJE FROM 07COMPROBACION LEFT JOIN 07XML ON 07COMPROBACION.id = 07XML.`ultimo_id` where ( 07COMPROBACION.NUMERO_EVENTO = "'.$NUMERO_EVENTO.'" ) ';
...
$VarSUBE = 'SELECT subTotal, UUID, MONTO_DEPOSITAR 
            ...
            AND 02SUBETUFACTURA.ID_RELACIONADO != ""';  // Condición para ID_RELACION lleno
...
$VarSUBE2 = 'SELECT subTotal, UUID, MONTO_DEPOSITAR 
            FROM 02SUBETUFACTURA 
            LEFT JOIN 02XML ON 02SUBETUFACTURA.id = 02XML.`ultimo_id` 
            WHERE 02SUBETUFACTURA.NUMERO_EVENTO = "'.$NUMERO_EVENTO.'" 
...
while ($ROWe2 = mysqli_fetch_array($QUERYSUBE2)) {
	...
	$VarSUBEpropina = 'SELECT  MONTO_PROPINA,IMPUESTO_HOSPEDAJE FROM 02SUBETUFACTURA LEFT JOIN 02XML ON 02SUBETUFACTURA.id = 02XML.`ultimo_id` where 02SUBETUFACTURA.NUMERO_EVENTO ="'.$NUMERO_EVENTO.'" AND 02SUBETUFACTURA.ID_RELACIONADO != ""';  // Condición para ID_RELACION lleno
...
while($ROWep=mysqli_fetch_array($QUERYSUBEP)){
	...
	$VarSUBEpropina2 = 'SELECT  MONTO_PROPINA,IMPUESTO_HOSPEDAJE FROM 02SUBETUFACTURA LEFT JOIN 02XML ON 02SUBETUFACTURA.id = 02XML.`ultimo_id` where 02SUBETUFACTURA.NUMERO_EVENTO ="'.$NUMERO_EVENTO.'" AND 02SUBETUFACTURA.VIATICOSOPRO = "PAGO A PROVEEDOR"';
...
$GTotalAvioComSube = $subTotalAVION + $subTotalCOMPROBACION + $subTotalSUBETUFACTURA + $subTotalSUBETUFACTURA2 +  $subTotalTikets+ $subTotalPROPINAOSERVICIO + $PorfaltaDeFactura+$subTotalVheiculo+$subTotalmaterial+$subTotapapeleria+$subTotaOFICINA+$subTotaBOTIQUIN+$subPERSONALtotal+$GTOTALBONO_VIATICO;
...
<tr>
<td style="background:#efdcf0;text-align:right;">COSTO POR NO DEDUCIBILIDAD</td>
