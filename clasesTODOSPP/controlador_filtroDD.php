...
$action = (isset($_POST["actionPP"])&& $_POST["actionPP"] !=NULL)?$_POST["actionPP"]:"";
if($action == "ajaxPP"){

	require(__ROOT6__."/class.filtroPP.php");
	$database=new orders();	
...
$per_page=intval($_POST["per_pagePP"]);
	$campos="*";
...
<?php 
if($database->plantilla_filtro($nombreTabla,"ID_RELACIONADO",$altaeventos,$DEPARTAMENTO)=="si"){ ?><th style="background:#c9e8e8;text-align:center">ID RELACIONADO </th>
<?php } ?>

<?php 
if($database->plantilla_filtro($nombreTabla,"VIATICOSOPRO",$altaeventos,$DEPARTAMENTO)=="si"){ ?><th style="background:#c9e8e8;text-align:center">TIPO DE COMPROBANTE</th>
<?php } ?>
...
<?php if($database->variablespermisos('','reembolsosquitar','ver')=='si'){ ?>
...
foreach ($datos as $key => $row) {
    ...
    // ✅ RECHAZADO → ROJO (prioridad máxima)
    if (isset($row['STATUS_RECHAZADO']) && $row['STATUS_RECHAZADO'] == 'si') {
        $fondo_existe_xml  = "style='background-color: #ff3c22'";
        $fondo_existe_xml2 = "style='background-color: #ff3c22'";
    }
    // Si ID_RELACIONADO está vacío → AMARILLO
    elseif (empty($row['ID_RELACIONADO'])) {
...
<?php if($database->plantilla_filtro($nombreTabla,"SUBTOTAL",$altaeventos,$DEPARTAMENTO)=="si"){ ?>
    ...
    $esRechazadoPP = (isset($row['STATUS_RECHAZADO']) && $row['STATUS_RECHAZADO'] == 'si');
...
<?php if($database->plantilla_filtro($nombreTabla,"DESCUENTO",$altaeventos,$DEPARTAMENTO)=="si"){ ?>
    ...
    if (!isset($esRechazadoPP) || !$esRechazadoPP) {
...
<?php if($database->plantilla_filtro($nombreTabla,"SUBTOTAL",$altaeventos,$DEPARTAMENTO)=="si"){ ?>
<td style="text-align:center"><strong style="font-size:16px">$<?php echo number_format($SUBTOTALG,2,'.',','); ?></strong></td>
<?php } ?>
...
<td class="celda-total">
   $<?php echo number_format(($SUBTOTALG22 + $SUBTOTALG2), 2, '.', ','); ?>
</td>
<?php } ?>
<?php if($database->plantilla_filtro($nombreTabla,"propina",$altaeventos,$DEPARTAMENTO)=="si"){ ?>
...
<td class="celda-total">
   $<?php echo number_format(($SUBTOTALG22 + $SUBTOTALG2 - $DESCUENTO12), 2, '.', ','); ?>
</td>
<?php } ?>
